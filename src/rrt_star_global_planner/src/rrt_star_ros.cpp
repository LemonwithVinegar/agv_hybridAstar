...
double start_time =0;
  ...
  {
    plan.clear();
  ROS_INFO("start.pose.position.x: %.2f, start.pose.position.y: %.2f",start.pose.position.x, start.pose.position.y);
    if(this->collision(start.pose.position.x, start.pose.position.y))
    {
      ROS_WARN("failed to get a path.start point is obstacle.");
      return false;
    }
    ROS_INFO("goal.pose.position.x: %.2f, goal.pose.position.y: %.2f", goal.pose.position.x, goal.pose.position.y);
    ...
    while (ros::ok() && nodes.size() + nodes_2.size() < max_nodes_num_)
    {
      ...
      if(is_connect_to_tree2)
      {
        ...
        path_pose.header.frame_id = "slam_map";
                                          ...
                                          GetPlanMode mode)
  {
    ...
    std::cout << "耗时：" << ros::Time::now().toSec() - start_time << "秒" << std::endl;
  }
