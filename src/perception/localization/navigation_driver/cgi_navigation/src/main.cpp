...
int main(int argc, char **argv){
    ...
    ros::Timer timer = nh.createTimer(ros::Duration(0.05),
        ...
        imu_pub.publish(imu_msg);    // 不是原始imu数据
   ...
   while (ros::ok()){
        ...
        while(ros::ok()){
			...
			gpgga_msg.lat = floor(atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str()) *0.01) + (atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str()) *0.01-floor(atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str()) *0.01))/0.6 ;
                ...
                // std::cout<< "gpgga_msg.lat:" << gpgga_msg.lat <<std::endl;
                // std::cout<< "gpgga_msg.lon:" << gpgga_msg.lon <<std::endl;
                 //std::cout<< "gpgga_msg.alt:" << gpgga_msg.alt <<std::endl;
                 //std::cout<< "gpgga_msg.qual:" << gpgga_msg.qual <<std::endl;
            }
            ...
            if(!strncmp(header.c_str(),"GPVTG",5)){
                ...
                while(ros::ok()){
                    ...
                    switch (index){ //现只填充了需要的数据
                    case 7:
                        gpvtg_msg.Speed_over_ground_km = atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str());
                        break;
                    default: 
                        break;    
                    }     
            ...
            if(!strncmp(header.c_str(),"PASHR",5)){
                ...
                while(ros::ok()){
                    ...
                    switch (index){ //现只填充了需要的数据
                    case 2:
                        pashr_msg.Heading = atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str());
                        break;
                    case 4:
                        pashr_msg.Roll = atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str());
                        break;
                    case 5:
                        pashr_msg.Pitch = atof(gpg_msg.substr(comma_position_front+1,comma_position_back-comma_position_front-1).c_str());
                        break;
                    default: 
                        break;    
                    }     
                 ...
                 //std::cout<< "pashr_msg.Heading:" << pashr_msg.Heading <<std::endl;
                // std::cout<< "pashr_msg.Roll:" << pashr_msg.Roll <<std::endl;
                // std::cout<< "pashr_msg.Pitch:" << pashr_msg.Pitch <<std::endl;
            }  
