...
//--->
bool reboot(custom_msgs::SLAMReboot::Request  &req,
                               custom_msgs::SLAMReboot::Response &res) {
    res.isSlamReboot = true;
    std::cout<< "occupancy_grid_node ready to reboot !!!" << std::endl;
    ros::shutdown();
    return true;
}
//<---
...
class Node {
 ...
 private:
  ...
  //--->
  ros::ServiceServer rebootService;
  //<---
  absl::Mutex mutex_;
      ...
      occupancy_grid_publisher_timer_(
          node_handle_.createWallTimer(::ros::WallDuration(publish_period_sec),
                                       &Node::DrawAndPublish, this)) {
                                         //--->
                                          rebootService = node_handle_.advertiseService("grid_reboot", cartographer_ros::reboot);
                                          //<---
                                       }
