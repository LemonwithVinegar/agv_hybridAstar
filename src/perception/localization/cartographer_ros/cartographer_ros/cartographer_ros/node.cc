...
//add 2022/3/30
  LOG(INFO) << "SLAM will save trajectoty to {->SEED_HOME/data/slam/slam_trajectory.txt<-}";
	const char* env_ptr = std::getenv("SEED_HOME");
        if(env_ptr == nullptr){
            ROS_FATAL("exception happened while reading env variable \"SEED_HOME\" ");
            exit(-1);
        }
 	std::string files = env_ptr; 
	std::fstream fout; 
	fout.open(files+"/data/slam/slam_trajectory.txt", std::ios::out);
	for( uint32_t i=0; i<response.trajectory.size(); i++ ){
		double x = response.trajectory[i].pose.position.x;
		double y = response.trajectory[i].pose.position.y;
		fout << x << " " << y << std::endl;
	}
  LOG(INFO) << "SLAM save trajectoty finished!" ;
//--------------
  return true;
}
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      if (node_options_.publish_to_tf) {      //true
        if (trajectory_data.trajectory_options.provide_odom_frame) {    //use odom
          ...
          // tf_broadcaster_.sendTransform(stamped_transforms);     //禁止发布tf
        } else {    // 未提供odom
          stamped_transform.header.frame_id = node_options_.map_frame;
          stamped_transform.child_frame_id =
              trajectory_data.trajectory_options.published_frame;
          ...
          // tf_broadcaster_.sendTransform(stamped_transform);  //禁止发布tf, 定位切换的时候注释
        }
      }
      if (node_options_.publish_tracked_pose) {   // 定位话题发布
