...
void SimpleLocalization::onNaviMsgRecvd(const custom_msgs::NaviData::ConstPtr &msg) 
{
    ...
    if (Is_slam_start != 0) {        //TASK already start
        ...
        if ( Is_slam_start==1 && slam_origin_get_once==false )    
        {
            ...
            try {   
                transformStamped = tf_buffer_ptr->lookupTransform("map", "slam_origin", ros::Time(0)); //map是slam map
                // transformStamped.header.frame_id = "gnss_map"; 
                // transformStamped.child_frame_id = "map"; 
                // static_br.sendTransform(transformStamped); 
                std::string target_link = env_p;
                fout4.open(target_link + "/data/Tgo_so.txt", std::ios::out); // gnss原点到slam原点的坐标关系

                if (0) { // 如果是地图匹配模式  MAP_MATCHING
                         // 需要写入Tgo_so.txt里面的数据
        ...
        // if (slam_origin_get_once==true && Is_slam_start==3 && (gnss_status!=42 || gnss_status!=52)) 
        {
            ...
            cur_pose.theta = slam_pose.theta;        // 可以注释掉：考虑直接使用惯导的偏航
            // cur_pose.theta =  0.5*(1-0.02)*(cur_pose.theta + slam_pose.theta)  + 0.02*slam_pose.theta;      // 互补滤波器，融合slam与gnss的偏航
            // yawAngle = Eigen::AngleAxisd( map_yaw-aftYaw, Eigen::Vector3d::UnitZ() );  
