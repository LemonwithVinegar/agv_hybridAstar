...
SimpleLocalization::SimpleLocalization()
{
    ...
    if(is_save_trajectory){
        ...
        fout.open(home_path+"/data/navigation/trajectory.txt",std::ios::out);//map save location
    }
...
void SimpleLocalization::onNaviMsgRecvd(const custom_msgs::NaviData::ConstPtr &msg)
{
    ...
    // std::cout << quaternion.x() << " " << quaternion.y() << " " << quaternion.z() << " "
    //             << quaternion.w() << std::endl;
    // std::cout << cur_pose.x << " " << cur_pose.y << std::endl;

    if(is_display_trajectory){//显示（保存）base_link坐标系原点在map坐标系中的轨迹
        try{
