...
GroundPlaneFit::GroundPlaneFit() : node_handle_("~") {
  ...
  node_handle_.param("sensor_height", sensor_height_, 0.4);
...
void GroundPlaneFit::grid_map_filter_(const pcl::PointCloud<VPoint>::ConstPtr in_cloud_ptr){
  ...
  for (size_t i = 0; i < in_cloud_ptr->size(); ++i){
    ...
    if (tmp_pt.x * 100 < (FRONT_SCAN_RANGE - 0.0001) && tmp_pt.x * 100 > -(REAR_SCAN_RANGE - 0.0001) && fabs(tmp_pt.y) * 100 <( LEFT_SCAN_RANGE - 0.0001)) {
...
void GroundPlaneFit::rslidar_callback_(
 ...
 pcl::PointCloud<pcl::PointXYZI> rawCloudIn; 
  ...
  for (size_t i = 0; i < rawCloudIn.points.size(); i++) {
    ...
    // // std::cout << "!!!!!!!!!!!!" << rawCloudIn.points[i].ring << std::endl;
    // // std::cout << "@@@@@@@@@@@@" << rawCloudIn.points[i].intensity << std::endl;
    // // std::cout << "$$$$$$$$$$$$" << rawCloudIn.points[i].timestamp << std::endl;
    ...
    // }
    if (point.x > -10.0 && point.x < 30.0 && point.y > -5.0 && point.y < 5.0 && point.z > -1.2 * sensor_height_ && point.z < 1.0) {
  ...
  for (size_t i = 0; i < laserCloudIn->points.size(); i++) {
    if (laserCloudIn->points[i].x > -0.8 && laserCloudIn->points[i].x < 0.5 &&
        abs(laserCloudIn->points[i].y) < 0.5) {
  ...
  float seg_step = 40.0 / num_seg_;
  float seg_step_y = 20.0 / num_seg_y_;
  for (int seg_x = 0; seg_x < num_seg_; seg_x++) {
    for (int seg_y = 0; seg_y < num_seg_y_; seg_y++) {
      ...
      seg_pass.setFilterLimits(-10.0 + seg_step * seg_x,
                               -10.0 + seg_step * (seg_x + 1));
