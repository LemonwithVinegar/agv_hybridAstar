...
uint16_t rebeam(uint16_t &beam, uint16_t &lines) 
{
  if(lines == 32)
  {
    switch (beam) {
...
GroundPlaneFit::GroundPlaneFit() : node_handle_("~") {
  ...
  node_handle_.param("half_road_width", half_road_width_, 10.0);
  ...
  node_handle_.param("sensor_height", sensor_height_, 0.46);
  ...
  node_handle_.param("th_dist", th_dist_, 0.4);
...
void GroundPlaneFit::estimate_plane_(void) {
  ...
  //std::cout << normal_(0.0)  << normal_(0.1) << normal_(0.2) <<  d_ << std::endl;
  float dist_to_edge = fabs(sqrt((normal_(0.0)*normal_(0.0))+(normal_(0.1)*normal_(0.1))+(normal_(0.2)*normal_(0.2))));
  float distance = d_/dist_to_edge;
  //std::cout <<"dist_to_edge::"  << dist_to_edge << "distance::"<<  distance << std::endl;
...
void GroundPlaneFit::grid_map_filter_(const pcl::PointCloud<VPoint>::ConstPtr in_cloud_ptr){
  ...
  for (size_t i = 0; i < in_cloud_ptr->size(); ++i){
    ...
    if (abs(tmp_pt.x) * 100 < FRONT_SCAN_RANGE && abs(tmp_pt.y) * 100 < LEFT_SCAN_RANGE) {
...
void GroundPlaneFit::rslidar_callback_(
  ...
  for (size_t i = 0; i < rawCloudIn.points.size(); i++) {
    ...
    // uint16_t beam = i / 2028;
    // //按从远到近排列激光束
    // point.ring = rebeam(beam, lidar_lines);
    ...
    if (point.x > -40.0 && point.x < 40.0 && point.y > -10.0 && point.y < 10.0 && point.z > -0.25 && point.z < 0.1) {
  ...
  for (size_t i = 0; i < laserCloudIn->points.size(); i++) {
    if (laserCloudIn->points[i].x > -1 && laserCloudIn->points[i].x < 1.5 &&
  ...
  for (int seg_x = 0; seg_x < num_seg_; seg_x++) {
    for (int seg_y = 0; seg_y < num_seg_y_; seg_y++) {
      ...
      seg_pass.setFilterLimits(-40.0 + seg_step * seg_x,
                               -40.0 + seg_step * (seg_x + 1));
      seg_pass.filter(*seg_laserCloudIn);

      seg_pass.setInputCloud(seg_laserCloudIn);
      seg_pass.setFilterFieldName("y");
      seg_pass.setFilterLimits(-half_road_width_ + seg_step_y * seg_y,
                               -half_road_width_ + seg_step_y * (seg_y + 1));
      seg_pass.filter(*seg_laserCloudIn);
